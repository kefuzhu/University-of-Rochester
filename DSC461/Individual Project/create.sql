...
CREATE TABLE User(ID VARCHAR(40) PRIMARY KEY, # awk -F "<>" '{print $1}' user.dat | wc -L yields 34. So the longest userID has 34 characters. To be conservative, set the limit to 50 characters
				  rating INT,
				  location TEXT,
				  country TEXT);

-- Create Table Item if not existed
CREATE TABLE Item(ID INT(10) PRIMARY KEY,
				  name TEXT,
...
CREATE TABLE Category(itemID INT(10),
...
CREATE TABLE Bid(itemID INT(10),
			     userID VARCHAR(40),
			     _time DATETIME,
			     amount DOUBLE,
...
-- Create Table CurrentTime if not existed
